...
type StripedRSService struct {
	encoder *encoder.RSEncoder
...
func MakeStripedRSService(pool *dir.VolumeConnectionPool) *StripedRSService {
	service := &StripedRSService{}
	service.pool = pool
	service.bufPool = common.MakeIOBufferPool()

	volume := pool.GetVolume()
	parameter := volume.Parameter
	service.encoder = encoder.MakeRSEncoder(parameter.K+parameter.Redundancy, parameter.K)
...
func (service *StripedRSService) WriteResponse(w io.Writer, objectID uint64, offset uint64, size uint64, PG dir.PlacementGroup, objectIndex index.ObjectIndex, broken int) {
	k := service.k
	...
	for now := firstStripe; now < finalStripe; now += iterationStripeNum {
		...
		if broken != -1 && broken<k {
			for j := range outputs {
				outputs[j] = service.bufPool.GetBuffer(int((end - now) * blockSize))
			}
...
func (service *StripedRSService) GetRecoverTasks(instruct dir.MoveInstruct) []dir.RecoveryTask {
	PG:=instruct.PG
...
func (service *StripedRSService) GenerateParity(pg dir.PlacementGroup) {
	return
}

func (service *StripedRSService) Put(objectID uint64, data common.IOBuffer, PG dir.PlacementGroup, objectIndex index.ObjectIndex) {
	n := service.k + service.redundancy
	k := service.k
	index := objectIndex.Stripe
	blockSize := service.blockSize
...
func (service *StripedRSService) repairBlock(pg dir.PlacementGroup, broken int, blockId int16, offset, size uint64) common.IOBuffer {
	...
	service.encoder.Encode(data, decoded)
	for j := 0; j < n; j++ {
		if j != broken {
			data[j].Unref()
		}
...
func (service *StripedRSService) Recovery() {
