...
type HitchhikerService struct {
	minBlockEncoder *encoder.RSEncoder
	encoder         *encoder.HHEncoder
...
func MakeHitchhikerService(pool *dir.VolumeConnectionPool) *HitchhikerService {
	service := &HitchhikerService{}
	...
	service.encoder = encoder.MakeHHEncoder(service.k+service.redundancy, service.k)
...
func (service *HitchhikerService) WriteResponse(w io.Writer, objectID uint64, offset uint64, size uint64, PG dir.PlacementGroup, objectIndex index.ObjectIndex, broken int) {
...
func (service *HitchhikerService) RepairBlocks(blockId int, offset uint64, size uint64, pg dir.PlacementGroup, broken int, shuffled bool) []common.IOBuffer {
...
func (service *HitchhikerService) RegenerateObject(PG dir.PlacementGroup, geoIndex *index.GeometricIndex, offset, size uint64, broken int) chan common.IOBuffer {
	n := service.k + service.redundancy
	args := make([]stor.GetBatchDataArgs,n)
	...
	go func() {
		...
		for _, block := range geoIndex.Blocks {
			...
			if currentOff+block.Size >= offset && currentOff < offset+size {
				...
				for i, off := range offs {
					args[i].Offset = append(args[i].Offset, off.Offset)
					args[i].Size = append(args[i].Size, uint32(off.Size))
					args[i].BlockId = append(args[i].BlockId, block.BlockID)
					args[i].PGId = int(PG.PGId)
				}
		...
		for i := 0; i < n; i++ {
			if i != broken {
				...
				chans[i] = getDataBySizes(&args[i], service.pool.GetConnectionByBrick(PG.Bricks[i]), service.bufPool, sizes[i], readNext[i])
			}
		}
...
func (service *HitchhikerService) RegenerateBlock(blockId int, offset uint64, size uint64, broken int, PG dir.PlacementGroup) common.IOBuffer {
	n := service.k + service.redundancy
	...
	for j := 0; j < n; j++ {
		if j != broken {
			go func(j int) {
				...
				args.BlockId = append(args.BlockId, int16(blockId))
				args.Offset = append(args.Offset, offs[j].Offset)
				args.Size = append(args.Size, uint32(offs[j].Size))
...
func (service *HitchhikerService) Get(offset uint64, size uint64, PG dir.PlacementGroup, objectIndex index.ObjectIndex) chan common.IOBuffer {
...
func (service *HitchhikerService) getMaxSize(blockID int16, pg dir.PlacementGroup) uint64 {
	k := service.k
	sizes := make([]uint64, k)
	var wg sync.WaitGroup
	for i := 0; i < k; i++ {
...
func (service *HitchhikerService) GenerateParity(PG dir.PlacementGroup) {
	k := service.k
...
func (service *HitchhikerService) Put(objectID uint64, data common.IOBuffer, PG dir.PlacementGroup, objectIndex index.ObjectIndex) {
	geoIndex := objectIndex.Geometric
	var wg sync.WaitGroup
...
func (service *HitchhikerService) GetRecoverTasks(instruct dir.MoveInstruct) []dir.RecoveryTask {
...
func (service *HitchhikerService) Recovery() {
