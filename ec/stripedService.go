...
func MakeStripedMSRService(pool *dir.VolumeConnectionPool) *StripedMSRService {
...
func (service *StripedMSRService) WriteResponse(w io.Writer, objectID uint64, offset uint64, size uint64, PG dir.PlacementGroup, objectIndex index.ObjectIndex, broken int) {
	k := service.k
...
func (service *StripedMSRService) GetRecoverTasks(instruct dir.MoveInstruct) []dir.RecoveryTask {
	PG:=instruct.PG
...
func (service *StripedMSRService) GenerateParity(pg dir.PlacementGroup) {
	return
}

func (service *StripedMSRService) Put(objectID uint64, data common.IOBuffer, PG dir.PlacementGroup, objectIndex index.ObjectIndex) {
	n := service.k + service.redundancy
	k := service.k
	index := objectIndex.Stripe
	blockSize := service.blockSize
...
func (service *StripedMSRService) repairBlock(pg dir.PlacementGroup, broken int, blockId int16, offset, size uint64) common.IOBuffer {
	var offs []encoder.OffAndSize

	for off :=uint64(0);off<size;off += service.blockSize {
		localOffs := service.encoder.GetRegenerateOffset(broken, offset + off, service.blockSize)
		offs = append(offs,localOffs...)
	}
...
func (service *StripedMSRService) Recovery() {
